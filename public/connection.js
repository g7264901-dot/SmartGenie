...
async function getGeneome() {
    try {
        ...
        if (!userData.isExist) { return {address: currentAccount, id: 0, referrals: []}; }
        const referralTree = {address: currentAccount, id: Number(userData.id), referrals: []};
            ...
            try {
                ...
                const level2Promises = data.referral.map(async (childAddr) => {
                    try {
                        ...
                        console.warn("Failed to fetch level 2 user:", childAddr, err.message);
                        return null;
                    }
                });
        ...
        referralTree.referrals = (await Promise.all(level1Promises)).filter(Boolean);
...
async function getIncomeDash() {
    ...
    const totEarns = dirRefIncData.dirRefIncome + teamBonusData.teamBonus + levelTotalData.lvlTotal;

    return({dirRefInc: dirRefIncData.dirRefIncome,
...
async function getDirectRefIncome() {
    try{
        ...
        if(directReferralCount >=1 && directReferralCount <=3) { incomeRate = 0.014; }
        else if(directReferralCount >3) { incomeRate = 0.018; }
        else{ return ({dirRefCnt: 0, dirRefCalc: "0 X 0.014 = 0", dirRefIncome: 0}); }

        const rawRefIncome = directReferralCount * incomeRate;
        const directReferralIncome = parseFloat(rawRefIncome.toFixed(5));
        
        return ({dirRefCnt: directReferralCount,
    ...
    try{
        ...
        for(i = 1; i <= 9; i++){
            if (i<=Number(userData.levelEligibility)) {
                ...
                const incomeCount = await contract.methods.getUserIncomeCount(currentAccount, i).call();
                calcIncome = web3.utils.toBN(incomeCount).mul(web3.utils.toBN(levelPrice));
...
async function getTeamDevDash(){
    try {
        ...
        return({dRefCnt: directReferralCount, indRefCnt: indirectReferralCount, refTotal: referralTotal});
    } catch (error) { console.error("Error fetching user data:", error); }
}
...
async function getPersonalDash() {
    try {
        ...
        const userData = await contract.methods.users(currentAccount).call();
        return ({userId: Number(userData.id), refId: Number(userData.referrerID), doj: unixToIndianDate(userData.joined)});
